    ...
    <div className={classes.setFreqComp}>
      <div>
        <Autocomplete
          ...
          getOptionLabel={(option: environments) => option.name.toString()}
          style={{ width: 300 }}
          renderInput={(params: any) => (
            <TextField {...params} label="環境の設定" variant="outlined" />
          )}
          onInputChange={(e, value) => {
            environments.forEach((element: environments) => {
              if (element.name.toString() === value) {
                ...
                dispatch({
                  type: "maxBlinkSet",
                  maxBlink: element.max_freq.max_frequency_data.max_blink,
                });
                dispatch({
                  type: "minBlinkSet",
                  minBlink: element.min_freq.min_frequency_data.min_blink,
                });
                dispatch({
                  type: "maxFaceMoveSet",
                  maxFaceMove:
                    element.max_freq.max_frequency_data.max_face_move,
                });
                dispatch({
                  type: "minFaceMoveSet",
                  minFaceMove:
                    element.min_freq.min_frequency_data.min_face_move,
                });
                dispatch({
                  type: "maxFreqIDSet",
                ...
                if (reFreq) {
                  ...
                  console.log(
                    element.max_freq.max_frequency_data.face_point_all
                  );
                  dispatch({
                  ...
                  dispatch({
                    ...
                    face_point:
                      element.min_freq.min_frequency_data.face_point_all,
                  });
                  if (
                    element.max_freq.max_frequency_data.face_angle_all !==
                    undefined
                  ) {
                    dispatch({
                  ...
                  if (
                    element.min_freq.min_frequency_data.face_angle_all !==
                    undefined
                  ) {
                    dispatch({
