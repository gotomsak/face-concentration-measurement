...
const ConcentrationEstimateComponent: React.FC<{
        ...
        setInterval(() => {
          detect(faceapi, video, canvas1, canvas2, cvnew).then((res) => {
            if (res === undefined) {
              ...
              console.log(
                "processiong: " + (endProcessing - startProcessing).toString()
              );
            }
            dispatch({
              type: "facePointSet",
            ...
            console.log("concent: " + (endConcent - startConcent).toString());
          });
        }, msSeparation)
      );
    }
  ...
  const ConcentrationCalculation = () => {
    if (sectionFaceMove.length >= separationNum) {
      ...
      if (frequency === "max") {
        if (
          store.getState().maxFaceMoveReducer < PointSum / separationNum ||
        ...
        if (
          store.getState().maxBlinkReducer < BlinkSum / separationNum ||
      ...
      if (frequency == "min") {
        if (
          store.getState().minFaceMoveReducer > PointSum / separationNum ||
        ...
        if (
          store.getState().minBlinkReducer > BlinkSum / separationNum ||
  ...
  const PreprocessingData = (newData: any, oldData: any) => {
    ...
    if (oldData.length > 0 && oldData.slice(-1)[0] !== undefined) {
      ...
      console.log("concentrationEstimate: " + store.getState().earLeftTReducer);
