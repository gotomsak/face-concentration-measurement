...
const SelectQuestionComponent: React.FC = () => {
  ...
  const [selectedQuestion, setSelectedQuestion] = useState<(number | string)[]>(
    []
  );
  ...
  return (
    ...
    >
      <DataGrid
        ...
        checkboxSelection
        onRowSelectionModelChange={(newSelect: GridRowSelectionModel) => {
          const newListData: (number | string)[] = newSelect.map(
        ...
        //   const newListData: (number | string)[] = newSelect.selectionModel.map(
        //     (value: number | string): number | string => {
        //       return value;
        //     }
        //   );
