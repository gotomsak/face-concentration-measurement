  ...
  useEffect(() => {
    store.subscribe(() => {
      ...
      if (store.getState().concReducer.c3.length !== 0) {
        setViewC3(
          Math.round(store.getState().concReducer.c3.slice(-1)[0] * 100) / 100
        );
        setViewC2(
          Math.round(store.getState().concReducer.c2.slice(-1)[0] * 100) / 100
        );
        setViewC1(
          Math.round(store.getState().concReducer.c1.slice(-1)[0] * 100) / 100
        );
        setViewW(
          Math.round(store.getState().concReducer.w.slice(-1)[0] * 100) / 100
        );
      }
    });
  }, []);
