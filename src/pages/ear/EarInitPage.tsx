...
const EarInitPage: React.FC = () => {
  ...
  useEffect(() => {
    if (cameraStop) {
      ...
      initEar({
        ...
        left_ear_list: store.getState().earLeftInitReducer.ear_left_init_list,
        left_ear_t: store.getState().earLeftInitReducer.ear_left_init_t,
        right_ear_list:
          store.getState().earRightInitReducer.ear_right_init_list,
        right_ear_t: store.getState().earRightInitReducer.ear_right_init_t,
        date: date,
      })
        .then((res: any) => {
          console.log(res);
  ...
  return (
    <div className={classes.root}>
      {startCheck ? (
        <EarInitComponent setFinishCheck={setFinishCheck}></EarInitComponent>
      ) : finishCheck ? (
